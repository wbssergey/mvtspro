/*
 *  Copyright (c) 2009, Zemtsov Sergey,  Wiztel INC

 *  custom rlm_sql.c module 
 *  with routing_query for Least Cost Route tariff lookup 
 *  in  postgres database 
 *  
 *  designed for NPA-NXX USA Voip subsystem, 
 *  external routing with Mvts I, Mvts Pro switches
 *
...
static CONF_PARSER module_config[] = {
	...
	{"postauth_query", PW_TYPE_STRING_PTR,
	 offsetof(SQL_CONFIG,postauth_query), NULL, ""},
        {"routing_query", PW_TYPE_STRING_PTR,
           offsetof(SQL_CONFIG,routing_query), NULL, ""},
...
request=request;
	...
	if (sql_set_user(inst, request, sqlusername, NULL) < 0)
     ...
     /* check if packet is routing request */
        isroute=0;

        x=request->packet->vps;
		while(x!=NULL) {
        
				strcpy(valuebuf,(x->strvalue));
                strcpy(valuebuf1,(x->name));
        if(strcmp(valuebuf,"xpgk-request-type=route")==0){
                   DEBUG2(valuebuf);  
                   isroute = 1;
                   break; 
                   };
        ...
        DEBUG2("routing packet is detected");
	...
	{
	/* force to accept -ignore authorize_check_query - if packet is  request for external routing */
		found=1;
	};
	/*
	...
	if (found > 0) {
		...
		radlog(L_DBG, "rlm_sql (%s): User %s not found in database ",
		       inst->config->xlat_name, sqlusername);
...
static int rlm_sql_postauth(void *instance, REQUEST *request) {
	...
	char		sqlusername[MAX_STRING_LEN];
        ...
        char valuebuf1[MAX_STRING_LEN];
        char valuebuf[MAX_STRING_LEN];
        char vgateway[MAX_STRING_LEN];
        char vip[MAX_STRING_LEN];
        char vsource[MAX_STRING_LEN];
        char vdestination[MAX_STRING_LEN]; 
        char vsrcin[MAX_STRING_LEN];
        char vsrcout[MAX_STRING_LEN];
        char vdstin[MAX_STRING_LEN];
        char vdstout[MAX_STRING_LEN];  
        char vusername[MAX_STRING_LEN];
        char vuserpassword[MAX_STRING_LEN];
        char vdisabled[MAX_STRING_LEN]; 
        char * a;
        char * b;
        int t; 
        int l;  
 ...
 /* quiet the compiler */
       ...
       x=request->packet->vps;
               while(x!=NULL) {
                ...
                strcpy(valuebuf1,(x->name));
               if(strcmp(valuebuf,"xpgk-request-type=route")==0){
			   DEBUG2("routing request is detected");
               DEBUG2(valuebuf);  
               isroute = 1;
                    };
                   
                 if(strncmp(valuebuf,"xpgk-src-number-in=",19)==0) {
                     strcpy(vsrcin,&valuebuf[19]);
					 DEBUG2("src-number-in:");
                     DEBUG2(vsrcin);
                             };
                  if(strncmp(valuebuf,"xpgk-src-number-out=",20)==0) {
                     strcpy(vsrcout,&valuebuf[20]);
   					  DEBUG2("src-number-out:");
                      DEBUG2(vsrcout);
                             };
                  if(strncmp(valuebuf,"xpgk-dst-number-in=",19)==0) {
                     strcpy(vdstin,&valuebuf[19]);
                      DEBUG2("dst-number-in:");
                      DEBUG2(vdstin);
                             };
                  if(strncmp(valuebuf,"xpgk-dst-number-out=",20)==0) {
                     strcpy(vdstout,&valuebuf[20]);
				     DEBUG2("dst-number-out:");
                     DEBUG2(vdstout);
                             };
                  if(strcmp(valuebuf1,"Calling-Station-Id")==0) {
                     strcpy(vsource,valuebuf);
				     DEBUG2("Calling-Station-Id:");
                     DEBUG2(vsource);
                     };
                  if(strcmp(valuebuf1,"Called-Station-Id")==0) {
                     strcpy(vdestination,valuebuf);
				     DEBUG2("Called-Station-Id:");
                     DEBUG2(vdestination);
                    };
                   if(strcmp(valuebuf1,"User-Name")==0) {
                     strcpy(vusername,valuebuf);
					 DEBUG2("User-Name:");
                     DEBUG2(vusername);
                    }; 
                    if(strcmp(valuebuf1,"User-Password")==0) {
                     strcpy(vuserpassword,valuebuf);
					 DEBUG2("User-Password:");
                     DEBUG2(vuserpassword);
                    };   
               x=x->next;
              };
	...
	if ((inst->config->postauth_query[0] == '\0')&&(inst->config->routing_query[0] == '\0'))
		return RLM_MODULE_NOOP;
 ...
 DEBUG("rlm_sql (%s): Processing sql_postauth", querystr);
...
/* If routing_query is not defined, we stop here */
        if (isroute == 1 ) {
        if (inst->config->routing_query[0] != '\0'){
                
        /* Expand variables in the query 
        memset(querystr, 0, MAX_QUERY_LEN);
        */
        radius_xlat(querystr, sizeof(querystr), inst->config->routing_query,
                    request, sql_escape_func);
        query_log(request, inst, querystr); 
        ...
        if (sqlsocket == NULL)
         ...
         DEBUG2("rlm_sql : Processing sql_routing");
        /* Process the query */
        if (rlm_sql_select_query(sqlsocket, inst, querystr)) {
                radlog(L_ERR, "rlm_sql (%s) in sql_postauth: Database query error - %s",
                       inst->config->xlat_name,
...
/* strcpy(valuebuf1,"xpgk_xrouting_username=wi_test/"); */

/* strcpy(valuebuf1,"wi_test/"); */

strcpy(valuebuf1,vusername);
strcat(valuebuf1,"/");
strcat(valuebuf1,vuserpassword);
...
pair = pairmake_any("Vendor-9-Attr-251", valuebuf,T_OP_EQ );

                  if (!pair)
                            DEBUG2("wrong creating pair");
     ...
     DEBUG2(row[0]);
     DEBUG("(%d)",MAX_STRING_LEN);
     strcpy(valuebuf,row[0]);
     DEBUG2(valuebuf);
     a=strchr(valuebuf,';');
     if (a==NULL) {
     DEBUG2("incorrect route field - 1 value");
     }
     else
     {
     strcpy(valuebuf1,(a+1));
     b=strchr(valuebuf1,';');
     if(b==NULL) {
     DEBUG2("incorrect route field - 2 values");
     };     
     strcpy(vdisabled,(b+1));
     (*b)=0;
     strcpy(vip,valuebuf1);
     (*a)=0;
     strcpy(vgateway,valuebuf);
     };

   
     if (strcmp(vdisabled,"0")==0) {

     strcpy(valuebuf1,vgateway);   
     strcat(valuebuf1,"/0/");

/*
     strcat(valuebuf1,vsource);
     strcat(valuebuf1,"/");
     strcat(valuebuf1,vdestination); 
     strcat(valuebuf1,"/");
*/

  strcat(valuebuf1,vsource);
     strcat(valuebuf1,"/");
     strcat(valuebuf1,vdstout); 
     strcat(valuebuf1,"/");

     strcat(valuebuf1,vsrcout);
     strcat(valuebuf1,"/");
     strcat(valuebuf1,vdestination); /* with no possible customer prefix */ 
     strcat(valuebuf1,"/");

  
   
     strcat(valuebuf1,vip);
     strcat(valuebuf1,":1720");
    /* strcat(valuebuf1,"/SIP"); */
...
pair = pairmake_any("Vendor-9-Attr-252", valuebuf,T_OP_EQ );
...
/*     strcat(valuebuf1,"4164321567/94012126932001/4164321567/12126932001"); */

/*

     strcpy(valuebuf1,vsrcin);
     strcat(valuebuf1,"/");
     strcat(valuebuf1,vdstout);
     strcat(valuebuf1,"/");
...
pair = pairmake_any("Vendor-9-Attr-250", valuebuf,T_OP_EQ );
