...
static int rlm_sql_postauth(void *instance, REQUEST *request) {
 ...
 /* quiet the compiler */
       ...
       x=request->packet->vps;
           ...
           while(x!=NULL) {
             vp_prints(valuebuf,MAX_STRING_LEN,x);
      if(strncmp(valuebuf,"User-Name = \"",strlen("User-Name = \""))==0)
{
                     strcpy(vusername,&valuebuf[strlen("User-Name = \"")]);
                  /*   strcpy(vusername,&vusername[1]); */
                     vusername[strlen(vusername)-1]=0;
};
 if(strncmp(valuebuf,"User-Password = \"",strlen("User-Password = \""))==0)
{
                     strcpy(vpassword,&valuebuf[strlen("User-Password = \"")]);
                /*     strcpy(vpassword,&vpassword[1]); */
...
strcpy(valuebuf,a+2);
       ...
       /*        strcpy(valuebuf,(x->strvalue));*/
             ...
             /*  break; */
                 ...
                 if(strncmp(valuebuf,"\"xpgk-src-number-in=",20)==0) {
                     strcpy(vsrcin,&valuebuf[20]);
                     vsrcin[strlen(vsrcin)-1]=0;
                             };
                  if(strncmp(valuebuf,"\"xpgk-src-number-out=",21)==0) {
                     strcpy(vsrcout,&valuebuf[21]);
                     vsrcout[strlen(vsrcout)-1]=0;
                             };
                  if(strncmp(valuebuf,"\"xpgk-dst-number-in=",20)==0) {
                     strcpy(vdstin,&valuebuf[20]);
                     vdstin[strlen(vdstin)-1]=0;
                             };
                  if(strncmp(valuebuf,"\"xpgk-dst-number-out=",21)==0) {
                     strcpy(vdstout,&valuebuf[21]);
                     vdstout[strlen(vdstout)-1]=0;
                             };
                  if(strcmp(valuebuf1,"Calling-Station-Id")==0) {
                     strcpy(vsource,valuebuf+1);
                     vsource[strlen(vsource)-1]=0;
                     };
                  if(strcmp(valuebuf1,"Called-Station-Id")==0) {
                     strcpy(vdestination,valuebuf+1);
                     vdestination[strlen(vdestination)-1]=0;
                    };  
               x=x->next;
              };
...
pair = pairmake_any("Vendor-9-Attr-251", valuebuf,T_OP_EQ );
     ...
     if(strlen(vsource)==0)  strcpy(vsource,"0");
     strcat(valuebuf1,vsource);
     
     strcat(valuebuf1,"/");
     
     if(strlen(vdstout)==0) strcpy(vdstout,"0");
     strcat(valuebuf1,vdstout); 
     
     strcat(valuebuf1,"/");

     if(strlen(vsrcout)==0) strcpy(vsrcout,"0");
     strcat(valuebuf1,vsrcout);
     
     strcat(valuebuf1,"/");
     
     if(strlen(vdstin)==0) strcpy(vdstin,"0"); 
     strcat(valuebuf1,vdstin); 
  ...
  DEBUG2("TimeStamp %s (%s) (%s)", ctime(&now),vusername,vdstout);
